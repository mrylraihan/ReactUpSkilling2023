...
import { counterActions, toggleActions } from '../store/index';
const Counter = () => {
  const numRef = useRef()
  // const state = useSelector(state => state.countSlice)
  const state = useSelector(state => state)
  const counter = useSelector(state => state.countSlice.count)
  const toggle = useSelector(state=>state.toggleSlice.toggle)
  const dispatch = useDispatch()

  const toggleCounterHandler = () => { dispatch(toggleActions.toggleCounter())};
